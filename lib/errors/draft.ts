...
export type ControlledDraftErrorCode = ControlledError<
  keyof typeof ErrorCodes
>;

export const isDraftErrorCode = <T>(
  error: unknown,
): error is T => {
  return (
...
export const ErrorCodes = {
  FAILED_DRAFT_CONTENT_UPDATE:
    'FAILED_DRAFT_CONTENT_UPDATE',
  FAILED_DRAFT_CREATION: 'FAILED_DRAFT_CREATION',
  FAILED_ALL_DRAFTS_RETRIEVAL:
    'FAILED_ALL_DRAFTS_RETRIEVAL',
  FAILED_DRAFT_CONTENT_RETRIEVAL:
    'FAILED_DRAFT_CONTENT_RETRIEVAL',
  FAILED_DRAFT_INFO_RETRIEVAL:
    'FAILED_DRAFT_INFO_RETRIEVAL',
...
> = {
  ...
  [ErrorCodes.FAILED_DRAFT_INFO_RETRIEVAL]:
    "An error ocurred while retrieving the requested draft's info. Please try again.",
  ...
  [ErrorCodes.UNAUTHORIZED_USER]:
    'You are not allowed to read/update this draft as you are not either the author or a collaborator.',
